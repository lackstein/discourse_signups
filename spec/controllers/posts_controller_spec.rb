...
describe PostsController do
  ...
  describe "polls" do
    ...
    it "should have at most 'SiteSetting.signup_maximum_options' options" do
      raw = "[poll]"
      (SiteSetting.signup_maximum_options + 1).times { |n| raw << "\n- #{n}" }
      raw << "[/poll]"
      ...
      expect(response).not_to be_success
      json = ::JSON.parse(response.body)
      expect(json["errors"][0]).to eq(I18n.t("poll.default_poll_must_have_less_options", max: SiteSetting.signup_maximum_options))
    end
