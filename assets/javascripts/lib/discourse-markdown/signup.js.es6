...
const DATA_PREFIX = "data-signup-";
const DEFAULT_SIGNUP_NAME = "signup";
...
export function setup(helper) {
  ...
  helper.replaceBlock({
    ...
    emitter(blockContents, matches) {
      ...
      // 1 - SIGNUP CONTAINER
      const container = ["div", { "class": "signup-container" }].concat(contents);
      signup.push(container);

      // 2 - SIGNUP INFO
      const info = ["div", { "class": "signup-info" }];
      ...
      info.push(["p",
                  ...
                  ["span", { "class": "info-text"}, I18n.t("signup.voters", {
                    count: 0,
                    one: helper.getOptions().signup_voters_text_one,
                    other: helper.getOptions().signup_voters_text_other
                  })]
                ]);
      ...
      // 3 - BUTTONS
      const buttons = ["div", { "class": "signup-buttons" }];
